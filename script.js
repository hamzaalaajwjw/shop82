  ...
  if(confirm("حذف الإعلان؟")) {
    ...
    db.ref("products/" + k).once('value', (snapshot) => {
      ...
      if (product) {
        ...
        // حذف المنتج
        db.ref("products/"+k).remove()
  .then(async () => {
    if (imageInput && imageInput.files.length > 0) {
      const productId = k || ref.key;
      for (let i = 0; i < imageInput.files.length; i++) {
        const file = imageInput.files[i];
        const path = `products/${productId}/${i+1}.jpg`;
        const { error } = await supabaseClient.storage.from('ads-images').upload(path, file, { upsert: true });
        if (!error) {
          const { data: urlData } = supabaseClient.storage.from('ads-images').getPublicUrl(path);
          imageUrls.push(urlData.publicUrl);
        }
      }
      await db.ref('products/' + productId).update({ images: imageUrls });
    }
...
function save(k){
  ...
  if (imageInput && imageInput.files.length > 0) {
    if (imageInput.files.length > 2) {
      alert('يسمح برفع صورتين فقط');
      return;
    }
    for (let i = 0; i < imageInput.files.length; i++) {
      const file = imageInput.files[i];
      if (!file.type.startsWith('image/')) {
        alert('الملفات يجب أن تكون صور فقط');
        return;
      }
    }
  }
