...
function displayImagePreview() {
    ...
    if (selectedImages.length === 0) {
        previewContainer.textContent = '<div style="color:#9ca3af; text-align:center; padding:20px;">لم يتم اختيار أي صور</div>';
        return;
    }
...
function loadProducts(){
  ...
  db.ref("products").once("value",snap=>{
    ...
    document.getElementById("products").textContent = finalHTML;

    renderPagination(totalPages);
  }).catch(error => {
    console.error("Error loading products:", error);
    document.getElementById("products").textContent = "<p class='empty'>حدث خطأ في تحميل الإعلانات</p>";
  });
}
...
function renderPagination(total){
  ...
  document.getElementById("pagination").textContent = html;
}
