// ===== تهيئة Supabase =====
const // removed (centralized in config/keys.js) 'https://dnclbdvdzvtdjpgxwnrl.supabase.co';
const // removed (centralized in config/keys.js) 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRuY2xiZHZkenZ0ZGpwZ3h3bnJsIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjcyMjY5OTcsImV4cCI6MjA4MjgwMjk5N30.alGg61mAPLLqLM2LlQRq2K2o_eOOnJwNuaIJiAXB7Wg';

const supabaseClient = // removed (centralized in config/keys.js)(supabaseUrl, supabaseKey);
...
// تهيئة Firebase
firebase.// initialized via services/firebase.js
const db = firebase.database();
const auth = firebase.auth();

// ===== متغيرات عامة =====
let userU/* REMOVED_KEY */ = null;
...
function loadProducts(){
  ...
  db.ref("products").once("value",snap=>{
      ...
      if((!c||p.category===c)&&p.name.toLowerCase().includes(s)){
        ...
        htmlCards.push({uid:p.uid,key:k,html:`
          <div class="card" onclick="showDetails('${k}')">
            ...
            ${sellerSection}
            <div class="actions">
              ${p.uid===userU/* REMOVED_KEY */?`<button class="edit" onclick="editProduct('${k}')">تعديل</button>
    ...
    if(userU/* REMOVED_KEY */){
      htmlCards = htmlCards.sort((a,b)=> b.uid===userU/* REMOVED_KEY */ ? 1 : -1);
    }
...
async function deleteProduct(k){ 
  ...
  // التحقق من الصلاحية
  if (product.uid !== userU/* REMOVED_KEY */ && !isAdmin) {
...
async function saveProduct(k){
  ...
  try {
    ...
    if (!k && userU/* REMOVED_KEY */) {
      const userSnapshot = await db.ref('users/' + userU/* REMOVED_KEY */).once('value');
      const userData = userSnapshot.val();
      
      if (userData) {
        const currentCount = userData.totalProducts || 0;
        await db.ref('users/' + userU/* REMOVED_KEY */).update({
          totalProducts: currentCount + 1,
...
function showDetails(k){
  db.ref("products/"+k).once("value",snap=>{
    ...
    document.getElementById("detailsContent").innerHTML = `
      ...
      ${p.uid === userU/* REMOVED_KEY */ ? `<p style="color:#38bdf8; font-size:14px; margin-top:10px;">هذا إعلانك</p>` : ""}
    `;
  ...
  supabase
    ...
    .then(({data})=>{
      ...
      document.getElementById("ad-content").innerHTML = `
        ...
        <a href="profile.html?id=${data.user_id}">حساب البائع</a>
      `;
    });
}
